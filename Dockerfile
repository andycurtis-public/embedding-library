...
COPY --chown=dev:dev . /workspace/embedding-library
RUN mkdir -p /workspace/build/embedding-library && \
    cd /workspace/build/embedding-library && \
    cmake /workspace/embedding-library && \
